...
def main(_):
  ...
  with tf.Graph().as_default(), tf.device('/cpu:0'):
    ...
    #      with tf.name_scope('%s_%d' % ('tower', i_gpu)):
            ...
            x_max = tf.clip_by_value(x_in_single + eps, -1.0, 1.0)
            x_min = tf.clip_by_value(x_in_single - eps, -1.0, 1.0)
            bs_this = x_in_single.shape[0]
            y = tf.constant(np.zeros([bs_single]), tf.int64)
            i = tf.constant(0)
            grad = tf.zeros_like(x_in_single)
            x_adv, _, _, _, _, _ = tf.while_loop(stop, graph, [x_in_single, y, i, x_max, x_min, grad])
    ...
    with tf.Session() as sess:
      ...
      for i in range(n_iter):
          names,adv_images = sess.run([filenames,x_adv])
